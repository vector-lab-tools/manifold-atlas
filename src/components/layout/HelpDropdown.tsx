...
const HELP_SECTIONS: HelpSection[] = [
  ...
  {
    title: "Concept-Guided Amplification in the Hegemony Compass",
    content: "The Hegemony Compass now ships with two amplification sliders (α_x and α_y) that let you steer the embeddings along each axis direction before the compass is projected. The default α = 0 is the unmodified manifold — the view the embedding model gives you by default. Positive α amplifies alignment with the axis's positive pole; negative α attenuates it; α = −1 removes the linear contribution of the axis concept entirely. The mathematical primitive is mean-difference / contrastive-activation-addition steering — take the difference between positive-pole and negative-pole centroids, normalise, and add α times that direction to each concept's embedding — which has been studied since Mikolov et al. (2013), formalised by Park, Choe & Veitch (2024) as the linear representation hypothesis, and applied to LLM steering by Rimsky et al. (2024). The visualisation framing follows the Latent Manipulator system (Bhowmick & Heer, CHI EA '26). The political point of the slider is sharper than the productivist framing in the CHI literature: when you watch concepts re-organise as α moves away from 0, you are seeing the default view restructure around an ideological dimension the manifold had naturalised. The Technical Dashboard reports axis polarisation (mean |x|, mean |y|, mean radial displacement) — Atlas's compass analogue of cluster purity from the Latent Manipulator paper. A polarisation reading that rises sharply with α is evidence the axis is doing genuine geometric work; a flat profile means the concepts don't load on that axis, or the axis was already saturated. Re-steering at a new α is free of API cost because the embedding cache holds the original vectors; the math is recomputed locally.",
  },
  {
